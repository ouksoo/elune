...
(function($, $win, $doc, undefined){
	...
	function bar(){
		...
		var c = 0, p = $('#preload_wrap'), b = p.find('.bg'), t = p.find('.txt_percent'), l = p.find('.bar i.on');
		...
		function callback(s){
			setTimeout(function(){
				...
				if(c < 8) callback(randomRange(10, 100));
	...
	fn = {
		...
		setResolution : function(){
			...
			winH = $win.height();
			if(winH < 700) $('body').addClass('low_resolution');
			else $('body').removeClass('low_resolution');
		}
	};
	$doc.ready(function(){
		var body;
